...
/// Regression test for a review finding on #52: an explicit `file://` URL
/// also parses to `gix::url::Scheme::File`, exactly like a bare local path
/// (`./modules`, `modules-typo`), but it must keep taking the clone branch
/// rather than being folded into `LocalPathNotFound`. `file://` is the only
/// way to select a specific branch of a *local* repository, since an
/// existing on-disk `--path` short-circuits before any git handling and
/// ignores `--branch`/`--depth` (see
/// `test_prepare_path_local_path_ignores_branch`). Uses the same no-network
/// trick as `test_prepare_path_invalid_branch_name`: `with_ref_name`
...
/// Regression test companion to the one above: distinguishes the explicit
/// `file://` case from the bare-path case that #52 is actually about. A
/// value with no scheme prefix at all (just a relative path that happens not
/// to exist) still parses to the same `gix::url::Scheme::File`, but it must
/// be reported as a missing local path, not sent to the clone branch -
/// otherwise the `file://` fix above would have silently widened to swallow
/// every local typo again.
#[test]
fn test_prepare_path_bare_scheme_file_path_is_still_reported_as_missing() {
    use clap::Parser;

    for missing in [
        "./nix-options-doc-issue-52-file-scheme-missing",
        "nix-options-doc-issue-52-file-scheme-missing",
    ] {
        let cli = Cli::parse_from(["program", "--path", missing, "--branch", "my branch"]);

        let err = prepare_path(&cli)
            .err()
            .unwrap_or_else(|| panic!("expected an error for path {missing:?}"));
        let msg = err.to_string();
        ...
        assert!(
            !msg.contains("Invalid branch or tag name"),
            "path {missing:?} must not take the clone branch: {msg}"
        );
    }
}
